        ...
        <section id='evaluate' className='evaluate'>
            <div className='evaluate__layout'>
                ...
                {isBelow1440 ? 
                    (
                        // Mobile layout
                        <motion.div className='evaluate__scroll-wrapper--mobile'
                        ref={wrapperRef}
                        initial={{x:0}}
                        animate={{x: isSlideRight ? slideOffset : 0}}
                        transition={{duration: 0.3}}>
                            <div ref={cardListRef} className='evaluate__card-list'>
                                {customerEvaluates.filter(i => i.index <= 3).map(item => (
                                    <CustomerCard 
                                    key={item.index} 
                                    index={item.index} 
                    ...
                    (
                        // Desktop layout
                        <motion.div className='evaluate__scroll-wrapper--desktop'
                        ref={wrapperRef}
                        initial={{x: 0}}
                        animate={{x: isSlideRight ? slideOffset : 0}}
                        transition={{duration: 0.3}}>
                ...
                <button onClick={() => setIsSlideRight(!isSlideRight)} className='evaluate_hint'>
                    向右查看更多 
                    <PiArrowRightLight className='evaluate__arrow'/>
                </button>
